...
def saveProductResultToExcel(String name, String image, String title, String brand, String price, String category, boolean testResult, String message) {
    ...
    //Nếu file đã tồn tại, mở file
        ...
        // Tạo sheet "Add Product Test Results"
        if (workbook.getSheet("Add Product Test Results") == null){
             sheet = workbook.createSheet('Add Product Test Results')
        } else {
            sheet = workbook.getSheet('Add Product Test Results')
        }
        fis.close()
    } else {
...
try {
    ...
    WebUI.click(findTestObject('Object Repository/Page_Rolex Admin/a_Thm Sn Phm'))

    WebUI.setText(findTestObject('Object Repository/Page_Rolex Admin/input_Tn Sn Phm_product'), name)
    WebUI.uploadFile(findTestObject('Object Repository/Page_Rolex Admin/input_file'), image)
    ...
    WebUI.click(findTestObject('Object Repository/Page_Rolex Admin/button_Submit'))
    ...
    if (isAlertPresent) {
        ...
        // Coi như thêm sản phẩm thành công nếu Alert xuất hiện
        saveProductResultToExcel(name, image, title, brand, price, category, true, "Thêm sản phẩm thành công (Alert: " + alertText + ")")
    } else {
        // Nếu không có Alert, coi như thêm sản phẩm thất bại
        saveProductResultToExcel(name, image, title, brand, price, category, false, "Thêm sản phẩm thất bại (Kiểm tra lại các trường thông tin)")
    }

} catch (Exception e) {
    // Kiểm tra nếu lỗi là do upload file không thành công
    if (e.getMessage().contains("Unable to upload file")) {
        saveProductResultToExcel(name, image, title, brand, price, category, false, "Thêm sản phẩm thất bại (Không thể tải file)")
    } else {
        // Nếu không phải lỗi upload file, ghi lại lỗi gốc
        saveProductResultToExcel(name, image, title, brand, price, category, false, 'Lỗi: ' + e.getMessage())
    }
} finally {
